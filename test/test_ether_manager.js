...
contract("ERC1155 EtherManager", accounts => {
  ...
  describe("Fee credit", async () => {
    ...
    it("depositFeeCredit", async () => {
      ...
      assert.equal(
        (await exchange.feesCredits(shareholder)).toNumber(),
        amount
      );
    ...
    it("withdrawFeeCredit", async () => {
      let balanceBefore = web3.utils.toBN(
        await web3.eth.getBalance(shareholder)
      );
  ...
  describe("Fee bonus", async () => {
    before(async () => {
      ...
      await exchange.addOrder(true, 5000, 100, {
        value: 5000 * 100,
        from: shareholder
      });
    });
    ...
    it("Only use its fee bonus", async () => {
      ...
      await exchange.addOrder(true, 1, 100, {
        value: 1 * 100,
        from: shareholder
      });
  ...
  describe("Ether withdrawal", async () => {
    before(async () => {
      ...
      await tokens.setApprovalForAll(exchangeAddress, true, { from: owner });
      exchange = await ERC1155ExchangeImplementationV1.at(exchangeAddress);
      await exchange.depositFeeCredit({
        value: ((price * amount) / 100) * 3,
        from: shareholder
      });
    });
    ...
    it("Owner withdraws", async () => {
      ...
      await exchange.addOrder(true, price, amount, {
        value: price * amount,
        from: shareholder
      }); // shareholder buy

      let actualPendingWithdrawals = web3.utils.toBN(
        await exchange.pendingWithdrawals(owner)
      );
      let expectedPendingWithdrawals = web3.utils
        .toBN(price)
      ...
      let gasUsed = web3.utils.toBN(
        (await exchange.withdraw({ from: owner })).receipt.gasUsed
      );
      let balanceAfter = web3.utils.toBN(await web3.eth.getBalance(owner));
      let gasSpend = gasUsed.mul(gasPrice);
      let expectedBalance = balanceBefore
        .sub(gasSpend)
        .add(expectedPendingWithdrawals);
      assert.isTrue(
