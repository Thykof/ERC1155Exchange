...
contract("ERC1155ExchangeOrderBook", accounts => {
  ...
  describe("Test getNextPrice", async () => {
    ...
    it("When only one price", async () => {
      ...
      // buy side
      await exchange.addOrder(true, 500, amount, {
        from: owner,
        value: 500 * amount
      });
    ...
    it("When 2 orders at different prices", async () => {
      ...
      assert.equal(
        (await exchange.getNextPrice(false, price)).toNumber(),
        price + 10
      );

      // buy side
      await exchange.addOrder(true, 490, amount, {
        from: owner,
        value: 490 * amount
      });
  ...
  describe("Test getOrderAtPrice", async () => {
    ...
    it("When one order", async () => {
      ...
      // buy side
      await exchange.addOrder(true, buyPrice, amount, {
        from: owner,
        value: buyPrice * amount
      });
      result = await exchange.getOrderAtPrice(true, buyPrice, 0);
    ...
    it("When 2 orders", async () => {
      ...
      // buy side
      await exchange.addOrder(true, buyPrice, amount + 1, {
        from: owner,
        value: buyPrice * (amount + 1)
      });
      result = await exchange.getOrderAtPrice(true, buyPrice, 0);
      checkOrder(result, buyPrice, amount, null, owner);
      result = await exchange.getOrderAtPrice(true, buyPrice, 1);
