...
contract("ERC1155", accounts => {
  ...
  describe("Simple upgrade", async () => {
    before(async () => {
      ...
      proxyExchangeAddress = logs.find(l => l.event == "TokenCreated")
      .args.exchangeAddress
      console.log("exchange: ", proxyExchangeAddress);
    })

    it("Check the proxy admin", async () => {
      let r = await web3.eth.getStorageAt(proxyExchangeAddress, '0xb53127684a568b3173ae13b9f8a6016e243e63b6e8ee1178d6a717850b5d6103')
    ...
    it("Upgrade", async () => {
      ...
      assert.equal(await token.tokenIdToProxyExchange(1), proxyExchangeAddress, { from: shareholder })

      newExchange = await MockERC1155ExchangeImplementationV2.at(proxyExchangeAddress)
