...
async function prepareContract() {
  ...
  const instance = new web3.eth.Contract(dataToken.abi, address);
...
async function demoUsage() {
  ...
  let r;
  r = await sendWithEstimateGas(
    instance.methods.setApprovalForAll(ADR2, true),
    ADR1
  );
...
async function createExchange(tokenId, feeRate) {
  ...
  const proxyInstance = await proxyExchange.new(
    exchangeAddress,
    web3.eth.abi.encodeFunctionCall(abiInitialize, [
      erc1155.address,
      tokenId,
      feeRate,
